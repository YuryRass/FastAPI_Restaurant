...
from app.submenu.shemas import SSubMenu, SUUID
...
async def add_submenu(menu_id: uuid.UUID, menu: SSubMenu, responce: Response):
    ...
    added_submenu: RowMapping = await SubmenuDAO.show_submenu(id=submenu["id"])
...
async def show_submenu_by_id(menu_id: uuid.UUID, submenu_id: uuid.UUID):
    submenu: RowMapping | None = await SubmenuDAO.show_submenu(
        id=submenu_id,
        menu_id=menu_id,
    )
    if not submenu:
...
async def show_submenus(menu_id: uuid.UUID):
    sub_menus = await SubmenuDAO.show_submenu(menu_id=menu_id)
...
async def update_submenu(
    ...
    submenu_res = await SubmenuDAO.show_submenu(id=updated_menu["id"])
